-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 171 · Costo congelado por ítem vendido / devuelto        ║
-- ║                                                                     ║
-- ║  PROBLEMA: items_venta no guarda el costo del momento. El CMV se    ║
-- ║  calcula con fn_costo() al vender pero se disuelve en un renglón    ║
-- ║  agregado del asiento (5.1.01) → el margen histórico REAL por SKU   ║
-- ║  es irrecuperable, y con inflación el "costo actual" retroaplicado  ║
-- ║  miente. Cada día sin esto es historia perdida para siempre.        ║
-- ║                                                                     ║
-- ║  DISEÑO: tabla satélite gateada por RLS, NO columna en items_venta  ║
-- ║  (items_venta es legible por cajeros vía REST; una columna de costo ║
-- ║  ahí violaría el blindaje de costos de las migs 050/096/161). La    ║
-- ║  satélite replica EXACTO el patrón de costos_producto: RLS con      ║
-- ║  fn_tiene_permiso('costos'), sin policy de escritura — solo         ║
-- ║  escriben las RPCs security definer.                                ║
-- ║                                                                     ║
-- ║  Reissue:                                                           ║
-- ║   · fn_crear_venta v13 = v12 (mig 163) ÍNTEGRA + 4 líneas: el       ║
-- ║     insert de items_venta devuelve el id y se congela el costo que  ║
-- ║     YA se calculaba para el CMV. Para combos: costo agregado de     ║
-- ║     los componentes ÷ cantidad (costo por unidad de combo).         ║
-- ║   · fn_crear_devolucion v-costo = v-iva (mig 163) ÍNTEGRA + el      ║
-- ║     espejo en items_devolucion.                                     ║
-- ║  El asiento contable NO cambia (cero riesgo contable). Firmas       ║
-- ║  intactas → create or replace limpio, sin DROP. Correr el chequeo   ║
-- ║  T1 después (0 duplicados).                                         ║
-- ║                                                                     ║
-- ║  Regla de lectura: fila ausente = costo desconocido al momento de   ║
-- ║  la venta (ventas pre-171, o producto sin costo cargado). Toda      ║
-- ║  métrica nueva trata "sin fila" como estimación, nunca como 0.      ║
-- ║                                                                     ║
-- ║  Después de correrla, actualizar types/database.ts:                 ║
-- ║   · Tables: costos_item_venta, costos_item_devolucion               ║
-- ║  REQUIERE: mig 163 corrida. Ejecutar UNA sola vez, COMPLETO.        ║
-- ╚════════════════════════════════════════════════════════════════════╝

-- ─────────────────────────────────────────────────────────────────────
-- 1. Satélites de costo congelado
-- ─────────────────────────────────────────────────────────────────────
create table if not exists public.costos_item_venta (
  item_venta_id  integer primary key
    references public.items_venta(id) on delete cascade,
  costo_unitario numeric(12,4) not null
);

comment on table public.costos_item_venta is
  'Costo unitario AL MOMENTO de la venta (lo mismo que sumó el CMV del
   asiento). Para combos: costo agregado de componentes por unidad de combo.
   Fila ausente = costo desconocido en ese momento (pre-mig-171 o sin costo
   cargado): las métricas lo tratan como estimación, no como 0. Gateada por
   RLS igual que costos_producto.';

create table if not exists public.costos_item_devolucion (
  item_devolucion_id integer primary key
    references public.items_devolucion(id) on delete cascade,
  costo_unitario     numeric(12,4) not null
);

comment on table public.costos_item_devolucion is
  'Costo unitario al momento de la devolución (espejo de costos_item_venta).';

alter table public.costos_item_venta enable row level security;
alter table public.costos_item_devolucion enable row level security;

drop policy if exists "costos_item_venta_select" on public.costos_item_venta;
create policy "costos_item_venta_select" on public.costos_item_venta
  for select to authenticated
  using ((select public.fn_tiene_permiso('costos')));

drop policy if exists "costos_item_devolucion_select" on public.costos_item_devolucion;
create policy "costos_item_devolucion_select" on public.costos_item_devolucion
  for select to authenticated
  using ((select public.fn_tiene_permiso('costos')));
-- Sin policy de escritura a propósito: SOLO escriben las RPCs definer.

-- ─────────────────────────────────────────────────────────────────────
-- 2. fn_crear_venta v13 · v12 (mig 163) ÍNTEGRA + costo congelado.
--    Cambios marcados con "v13".
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- 3. fn_crear_devolucion v-costo · v-iva (mig 163) ÍNTEGRA + espejo del
--    costo congelado en items_devolucion. Cambios marcados con "v-costo".
-- ─────────────────────────────────────────────────────────────────────
...
--
-- 1. Chequeo T1 de funciones duplicadas (debe devolver 0 filas):
--    select proname, count(*) from pg_proc p
--    join pg_namespace n on n.oid = p.pronamespace
--    where n.nspname = 'public' and proname like 'fn_%'
--    group by proname having count(*) > 1;
--
-- 2. Venta de prueba desde el POS → cada ítem con control de stock y costo
--    cargado debe tener su fila:
--    select iv.id, iv.producto_id, civ.costo_unitario
--    from items_venta iv
--    left join costos_item_venta civ on civ.item_venta_id = iv.id
--    order by iv.id desc limit 5;
--
-- 3. Con un usuario SIN permiso 'costos', el select directo a
--    costos_item_venta debe devolver 0 filas (RLS).
--
-- 4. El asiento de la venta de prueba debe seguir cuadrando (debe = haber)
--    y el renglón 5.1.01 debe coincidir con
--    Σ(cantidad × costo_unitario congelado) de esa venta.
-- ─────────────────────────────────────────────────────────────────────
