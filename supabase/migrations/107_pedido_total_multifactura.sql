-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 107 · pedido.total correcto con multi-factura            ║
-- ║                                                                     ║
-- ║  fn_guardar_factura_compra (v9, mig 095) hacía                      ║
-- ║    update pedidos set total = <total de ESTA factura>              ║
-- ║  → en un pedido con varias facturas, el total del pedido terminaba ║
-- ║  mostrando el de la ÚLTIMA factura cargada, no la suma. (Las deudas ║
-- ║  en cuentas_a_pagar siempre estuvieron bien; esto es solo el dato   ║
-- ║  de display del total del pedido.)                                  ║
-- ║                                                                     ║
-- ║  v10: idéntica a la v9 salvo el cálculo de pedidos.total, que pasa  ║
-- ║  a ser:  suma de TODAS las facturas cargadas del pedido            ║
-- ║        + suma de las deudas provisorias que todavía NO tienen       ║
-- ║          factura (su estimado).                                     ║
-- ║  Se recalcula DESPUÉS de marcar esta cuenta como tiene_factura=true ║
-- ║  para no contar dos veces la factura recién cargada.                ║
-- ║                                                                     ║
...
begin
  ...
  --   Se hace DESPUÉS del update de arriba (esta cuenta ya es
  --   tiene_factura=true), así no se cuenta dos veces la factura actual.
