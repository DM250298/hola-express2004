-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 027 · FASE 2 — POS Offline (idempotencia de ventas)      ║
-- ║                                                                     ║
-- ║  Cuando el POS vende sin conexión, la venta se guarda en una cola   ║
-- ║  local del navegador y se sincroniza al volver internet. Para que   ║
-- ║  un reintento de sincronización NO duplique la venta, cada venta    ║
-- ║  lleva un identificador único generado en el cliente (cliente_uuid).║
-- ║                                                                     ║
-- ║  fn_crear_venta pasa a recibir p_cliente_uuid: si ya existe una     ║
-- ║  venta con ese uuid, la devuelve tal cual (no inserta de nuevo).    ║
-- ║                                                                     ║
...
create unique index if not exists ventas_cliente_uuid_key
  on public.ventas (cliente_uuid)
  where cliente_uuid is not null;

-- 2. Reemplazar fn_crear_venta con la versión idempotente.
--    Se elimina la firma anterior (4 args) para no dejar overloads.
...
begin
  ...
  insert into public.ventas
    (turno_id, usuario_id, total, medio_pago, estado, cliente_uuid)
  values
    (p_turno_id, p_usuario_id, v_total, v_medio_principal, 'completada', p_cliente_uuid)
