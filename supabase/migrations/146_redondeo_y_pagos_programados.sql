-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 146 · Redondeo exacto + condonación de residuos +        ║
-- ║                  PAGOS PROGRAMADOS                                   ║
-- ║                                                                     ║
-- ║  Tres problemas reales de producción:                                ║
-- ║   · El total calculado no siempre da EXACTO como el papel de la     ║
-- ║     factura (redondeos por línea) → campo `redondeo` (+/−) en la    ║
-- ║     factura, que ajusta el total y asienta contra 5.2.10.           ║
-- ║   · Un pago que queda a centavos del total dejaba la deuda          ║
-- ║     'pendiente' con $0,01 colgado (caso cuenta #47) → los residuos  ║
-- ║     de hasta $1 se CONDONAN contra 5.2.10 y la deuda cierra sola.   ║
-- ║   · "Lo pago el viernes": pagos con fecha FUTURA quedan             ║
-- ║     PROGRAMADOS (tabla nueva, no debitan nada) y se ejecutan con    ║
-- ║     un click desde Finanzas › Cuentas a pagar.                      ║
-- ║                                                                     ║
-- ║  Piezas:                                                            ║
-- ║   1. facturas_compra.redondeo + tabla pagos_programados (RLS        ║
-- ║      finanzas) — sumar pagos_programados al reset_inicio_cero.     ║
-- ║   2. fn_pagar_cuenta v4 (misma firma, REPLACE): condona residuo     ║
-- ║      0 < r ≤ $1 (asiento Debe 2.1.01 / Haber 5.2.10) y devuelve     ║
-- ║      pago_id + condonado.                                           ║
-- ║   3. fn_guardar_factura_compra v16 (11º arg p_redondeo → DROP de    ║
-- ║      la v15 de 10 args): total incluye redondeo (asiento 5.2.10     ║
-- ║      según signo) + pagos con fecha futura → pagos_programados.     ║
-- ║      Compat: el frontend viejo llama sin p_redondeo → default 0     ║
-- ║      (esta migración puede correrse ANTES del deploy).              ║
-- ║   4. fn_ejecutar_pago_programado: ejecuta con fecha de HOY vía      ║
-- ║      fn_pagar_cuenta (guards de permiso/período/bóveda incluidos).  ║
-- ║                                                                     ║
-- ║  REQUIERE: migs 144 y 145 corridas.                                 ║
-- ║  Ejecutar UNA sola vez, COMPLETO, en el SQL Editor de Supabase.     ║
-- ╚════════════════════════════════════════════════════════════════════╝

-- ─────────────────────────────────────────────────────────────────────
-- 1a. Redondeo del comprobante (para clavar el total igual al papel)
-- ─────────────────────────────────────────────────────────────────────
alter table public.facturas_compra
  add column if not exists redondeo numeric(12,2) not null default 0;

comment on column public.facturas_compra.redondeo is
  'Ajuste (+/−) para que el total calculado coincida EXACTO con el impreso en el comprobante. Asienta contra 5.2.10 Diferencias por Pagos y Redondeos.';

-- ─────────────────────────────────────────────────────────────────────
-- 1b. Pagos programados ("lo pago el viernes": no debita hasta ejecutar)
-- ─────────────────────────────────────────────────────────────────────
create table if not exists public.pagos_programados (
  ...
  -- Fila de pagos_cuenta creada al ejecutar (FK lógica, sin constraint).
  pago_id           integer,
...
create index if not exists idx_pagos_programados_estado_fecha
  on public.pagos_programados (estado, fecha_programada);

alter table public.pagos_programados enable row level security;
do $$ begin
  create policy gate_rw on public.pagos_programados
...
-- ─────────────────────────────────────────────────────────────────────
-- 2. fn_pagar_cuenta v4 — condonación de residuos chicos + pago_id
--    Misma firma que la v3 (mig 144) → REPLACE limpio, sin drop.
-- ─────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- se condona contra 5.2.10 y la deuda cierra sola — nunca más un $0,01
  -- colgado como la cuenta #47. Un pago parcial de verdad (residuo > $1)
  -- no se toca.
...
-- ─────────────────────────────────────────────────────────────────────
-- 3. fn_guardar_factura_compra v16 — p_redondeo + pagos programados
--    Cambia la firma (11º arg) → drop de la v15 (10 args) primero.
-- ─────────────────────────────────────────────────────────────────────
drop function if exists public.fn_guardar_factura_compra(integer, integer, integer, date, boolean, uuid, jsonb, jsonb, numeric, jsonb);
...
begin
  ...
  -- ── Pagos integrados (v15: uno o varios) / PROGRAMADOS (v16) ─────────
  -- Fecha ≤ hoy → pago real (debita ya, misma transacción). Fecha FUTURA →
  -- queda PROGRAMADO en pagos_programados: no debita nada hasta ejecutarlo
  -- desde Finanzas › Cuentas a pagar.
  if p_pago is not null then
    ...
    if jsonb_array_length(v_pagos) > 0 then
      ...
      for v_pago in select * from jsonb_array_elements(v_pagos) loop
        ...
        v_pago_fecha := coalesce(nullif(v_pago->>'fecha', '')::date, current_date);
        ...
        if v_pago_fecha > current_date then
...
-- ─────────────────────────────────────────────────────────────────────
-- 4. fn_ejecutar_pago_programado — el click de "Ejecutar"
-- ─────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- La plata sale HOY (cuando se ejecuta), no en la fecha programada.
  -- fn_pagar_cuenta v4 valida permiso/período/bóveda y condona residuos.
