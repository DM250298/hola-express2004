-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 145 · fn_guardar_factura_compra v15: pagos MÚLTIPLES     ║
-- ║                                                                     ║
-- ║  El widget "Agregar pago" del modal de factura permite cargar       ║
-- ║  varios pagos en el mismo guardado (ej: mitad transferencia del     ║
-- ║  banco + mitad efectivo de la caja fuerte).                         ║
-- ║                                                                     ║
-- ║  v15 = v14 (mig 144) con UN solo cambio, en el bloque de pago del   ║
-- ║  final: p_pago acepta                                               ║
-- ║   · un OBJETO {cuenta_origen_id, monto, forma_pago?, comprobante?,  ║
-- ║     fecha?, nota?}  → compat v14 (el frontend viejo sigue andando,  ║
-- ║     esta migración puede correrse ANTES del deploy), o              ║
-- ║   · un ARRAY de esos objetos → un fn_pagar_cuenta por elemento,     ║
-- ║     EN ORDEN, dentro de la misma transacción.                       ║
-- ║                                                                     ║
-- ║  Los perform secuenciales componen solos: cada fn_pagar_cuenta      ║
-- ║  recalcula el aplicado desde pagos_cuenta (ve los inserts previos   ║
-- ║  de la misma transacción). Parciales acumulan; el pago que CRUZA    ║
-- ║  el total absorbe el sobrante (Debe 5.2.10) y deja 'pagada'.        ║
-- ║  BACKSTOP: un pago posterior a uno que ya dejó la cuenta 'pagada'   ║
-- ║  hace raise → aborta TODO atómicamente (la UI lo pre-valida).       ║
-- ║                                                                     ║
-- ║  La firma NO cambia (mismos 10 args) → create or replace limpio,    ║
-- ║  sin drop. Requiere la mig 144 corrida (fn_pagar_cuenta v3,         ║
-- ║  columnas de pagos_cuenta y cuenta 5.2.10).                         ║
-- ║                                                                     ║
...
declare
  ...
  -- Pagos previos + pago integrado (v14) / pagos múltiples (v15)
...
begin
  ...
  -- ── Pagos integrados (v15: uno o VARIOS) ────────────────────────────
  -- Corre al FINAL, con la deuda ya fijada en v_total, DENTRO de la misma
  -- transacción: si CUALQUIER pago falla (bóveda negativa, período, permiso,
  -- deuda ya cubierta), la factura y los demás pagos tampoco se guardan.
    ...
    if jsonb_array_length(v_pagos) > 0 then
      ...
      for v_pago in select * from jsonb_array_elements(v_pagos) loop
        ...
        perform public.fn_pagar_cuenta(
          ...
          coalesce(nullif(v_pago->>'fecha', '')::date, current_date),
...
-- La firma no cambió (create or replace conserva los grants de la 144), pero
-- se re-emiten por prudencia. El revoke FROM PUBLIC pisa el EXECUTE default.
