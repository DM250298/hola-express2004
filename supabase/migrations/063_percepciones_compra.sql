-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 063 · Percepciones sufridas en compras (IIBB / IVA)      ║
-- ║                                                                     ║
-- ║  1) Columnas de percepción en facturas_compra.                     ║
-- ║  2) Cuentas de "percepciones a favor" (activo) en el plan.         ║
-- ║  3) Reissue de fn_guardar_factura_compra: suma las percepciones al ║
-- ║     total/deuda y al asiento (Debe percepción a favor).            ║
-- ║                                                                     ║
...
alter table public.facturas_compra
  ...
  add column if not exists percepcion_otros numeric(12, 2) not null default 0;

-- ─── 2) Cuentas de percepciones a favor (activo corriente) ──────────
insert into public.plan_cuentas (codigo, nombre, tipo, imputable) values
  ('1.1.07', 'Percepciones IVA a favor', 'activo', true),
  ('1.1.08', 'Percepciones IIBB a favor', 'activo', true),
  ('1.1.09', 'Otras percepciones a favor', 'activo', true)
on conflict (codigo) do nothing;

-- ─── 3) fn_guardar_factura_compra v7 (con percepciones) ─────────────
drop function if exists public.fn_guardar_factura_compra(
  integer, integer, integer, date, boolean, uuid, jsonb
);
...
begin
  ...
  v_total := round(v_neto + v_iva_total + v_perc_iva + v_perc_iibb + v_perc_otros, 2);
  ...
  if v_total > 0 and v_cta_merc is not null and v_cta_iva_cred is not null and v_cta_prov is not null then
    ...
    values (v_asiento_id, v_cta_merc, v_neto, 0, v_orden);
    v_orden := v_orden + 1;
