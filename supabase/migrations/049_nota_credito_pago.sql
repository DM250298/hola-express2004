-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 049 · R2.2 — Usar nota de crédito como pago              ║
-- ║                                                                     ║
-- ║  fn_crear_venta v4: si un pago tiene medio_pago='nota_credito' con  ║
-- ║  su 'nc_codigo', valida y consume el saldo del vale dentro de la    ║
-- ║  misma transacción de la venta (atómico). No toca cuentas (no es    ║
-- ║  efectivo ni banco) y no se cuenta como efectivo en el cierre.      ║
-- ║                                                                     ║
...
begin
  ...
  for v_pago in select * from jsonb_array_elements(p_pagos) loop
    ...
    -- ── Nota de crédito: consumir el saldo del vale ──
