-- ╔══════════════════════════════════════════════════════════════════════╗
-- ║ 156 · Bonificadas (desc 100% / costo $0) NO pisan el costo          ║
-- ║                                                                      ║
-- ║ PROBLEMA: mercadería que el proveedor regala (renglón con descuento  ║
-- ║ 100%, costo landed $0). La v19 hacía fn_set_costo(prod, 0) igual →   ║
-- ║ costos_producto quedaba en $0, el CMV de las ventas siguientes salía ║
-- ║ $0 y el P&L inflaba el margen. proveedor_producto.costo, el margen   ║
-- ║ deducido y el reprecio mayorista arrastraban el mismo $0.            ║
-- ║                                                                      ║
-- ║ FIX = v20 (cuerpo v19 de la mig 154, íntegro, MISMA firma 12 args):  ║
-- ║  · fn_set_costo y proveedor_producto.costo solo con landed > 0       ║
-- ║    (bonificada → se CONSERVA el último costo real del producto)      ║
-- ║  · precio manual con landed $0: el margen del par coherente se       ║
-- ║    deduce contra el costo CONSERVADO (fn_costo), no contra $0        ║
-- ║  · precio_mayorista solo se reprecia con landed > 0                  ║
-- ║  · historial_costos ya tenía la guarda (landed > 0): sin cambios     ║
-- ║                                                                      ║
-- ║ ORDEN: correr DESPUÉS de 154 y 155. Misma firma → create or replace  ║
