-- ╔══════════════════════════════════════════════════════════════════════╗
-- ║ 154 · FIX: sobrecarga duplicada de fn_guardar_factura_compra        ║
-- ║                                                                      ║
-- ║ PROBLEMA: la mig 153 (listas mayorista) reemitió la función con la   ║
-- ║ firma VIEJA de 9 args (base v13/mig 138), sin saber que 144→148 la   ║
-- ║ habían llevado a 11/12 args (p_pago, p_redondeo, p_cuotas). Postgres ║
-- ║ trató la firma distinta como OTRA función → dos sobrecargas vivas y  ║
-- ║ PostgREST falla con "Could not choose the best candidate function".  ║
-- ║                                                                      ║
-- ║ FIX: borrar TODAS las sobrecargas y reemitir UNA sola = v19:         ║
-- ║   cuerpo v18 (mig 148, íntegro) + repriceo mayorista de la 153.      ║
-- ║                                                                      ║
-- ║ ORDEN: correr DESPUÉS de 148 y 149 (necesita cuotas_cuenta_pagar).   ║
-- ║ Verificación al final del archivo.                                   ║
-- ╚══════════════════════════════════════════════════════════════════════╝

drop function if exists public.fn_guardar_factura_compra(
  integer, integer, integer, date, boolean, uuid, jsonb, jsonb, numeric);
drop function if exists public.fn_guardar_factura_compra(
  integer, integer, integer, date, boolean, uuid, jsonb, jsonb, numeric, jsonb);
drop function if exists public.fn_guardar_factura_compra(
    ...
    if p_afecta_precio_venta then
      update public.productos
        set precio_venta = v_precio_con_iva,
            margen = v_margen,
            -- v19 (de mig 153): el precio MAYORISTA se recalcula SIEMPRE desde SU
            -- margen con el mismo costo landed. Sin margen_mayorista → queda igual.
...
-- ── VERIFICACIÓN: debe devolver UNA sola fila (12 args) ────────────────
