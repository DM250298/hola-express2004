-- ╔══════════════════════════════════════════════════════════════════════╗
-- ║ 157 · FIX bonificadas: costo previo NULL reventaba el guardado      ║
-- ║                                                                      ║
-- ║ PROBLEMA: la v20 (mig 156) le pasa el costo CONSERVADO (fn_costo)    ║
-- ║ a fn_margen_desde_precio cuando el renglón es bonificado (landed $0  ║
-- ║ + precio manual). Pero fn_costo (mig 051, language sql) devuelve     ║
-- ║ NULL si el producto no tiene fila en costos_producto (alta al vuelo  ║
-- ║ sin factura previa) — el coalesce interno no corre sin fila — y      ║
-- ║ fn_margen_desde_precio hace raise con p_costo NULL (mig 125:47):     ║
-- ║ "costo >= 0 (recibido <NULL>)" → aborta TODA la factura.             ║
-- ║                                                                      ║
-- ║ Además la v20 dejó un agujero: bonificada SIN precio tipeado (modo   ║
-- ║ margen) → fn_precio_venta(0,…) = $0 y el update de productos pisaba  ║
-- ║ precio_venta con $0 y margen con 0 (vendible a $0 en el POS).        ║
-- ║                                                                      ║
-- ║ FIX = v21 (cuerpo v20 de la mig 156, íntegro, MISMA firma 12 args):  ║
-- ║  · v_costo_ant := coalesce(fn_costo(...), 0)                         ║
-- ║  · precio_venta y margen solo se actualizan con precio > 0           ║
-- ║    (un renglón con costo real siempre da precio > 0: solo cambia     ║
-- ║    el caso bonificada sin precio)                                    ║
-- ║                                                                      ║
-- ║ ORDEN: correr DESPUÉS de la 156. Misma firma → create or replace     ║
-- ║ directo, sin drops. Verificación al final del archivo.               ║
-- ╚══════════════════════════════════════════════════════════════════════╝
...
begin
  ...
  for v_linea in select * from jsonb_array_elements(p_lineas) loop
    ...
    if v_precio_manual is not null and v_precio_manual > 0 then
      ...
      -- v20: bonificada (landed $0) → el margen se deduce contra el costo
      -- CONSERVADO del producto, no contra $0 (que daba margen 0 incoherente).
      v_precio_con_iva := round(v_precio_manual, 2);
      v_margen := public.fn_margen_desde_precio(
        case when v_costo_landed > 0 then v_costo_landed else v_costo_ant end,
    ...
    if p_afecta_precio_venta then
      update public.productos
        -- v21: bonificada sin precio tipeado → fn_precio_venta(0,…) da $0;
        -- no se pisa el precio/margen vigentes con eso (vendible a $0).
        set precio_venta = case when v_precio_con_iva > 0 then v_precio_con_iva else precio_venta end,
            ...
            -- v20: con landed $0 (bonificada) tampoco se reprecia.
            precio_mayorista = case
              when margen_mayorista is not null and v_costo_landed > 0
