-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 213 · fn_importar_productos v5 — IVA de compra aparte    ║
-- ║                                                                     ║
-- ║  La v4 (126) escribía la única columna "iva" del Excel en           ║
-- ║  iva_venta Y en iva_compra, y el export solo trae el de venta: un   ║
-- ║  export → import pisaba el IVA de compra de la ficha (las harinas   ║
-- ║  al 10,5 volvían al 21 de venta… o al revés).                       ║
-- ║                                                                     ║
-- ║  v5: columna nueva opcional "iva_compra".                           ║
-- ║    · update: iva_compra = coalesce(iva_compra del archivo, actual)  ║
-- ║      → "iva" ya NO toca el de compra.                               ║
-- ║    · alta:   iva_compra = coalesce(iva_compra, iva, 21)             ║
-- ║  Resto idéntico a la 126. Firma idéntica → CREATE OR REPLACE.       ║
-- ║                                                                     ║
...
begin
  ...
  for v_fila in select * from jsonb_array_elements(p_filas) loop
    begin
      ...
      v_iva_compra_in := nullif(v_fila->>'iva_compra','')::numeric;
      ...
      if v_prod_id is not null then
        update public.productos set
          ...
          iva_compra      = coalesce(v_iva_compra_in, iva_compra),
        ...
        insert into public.productos (
          ...
          coalesce(v_iva_compra_in, v_iva_in, 21),
...
-- Verificación: select to_regprocedure('public.fn_importar_productos(jsonb)');
