-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 168 · El IVA corregido en la factura vuelve a la ficha   ║
-- ║                                                                     ║
-- ║  fn_guardar_factura_compra v24 (cuerpo íntegro de la v23, mig 166;  ║
-- ║  MISMA firma de 12 args → create or replace, sin drop).             ║
-- ║                                                                     ║
-- ║  Problema: el modal de carga de factura dejó de hardcodear el 21 y  ║
-- ║  ahora siembra iva_venta / iva_compra desde la ficha del producto,  ║
-- ║  pero el RPC nunca escribía esas dos columnas: corregir la alícuota ║
-- ║  en un renglón se perdía y la próxima factura volvía a proponer la  ║
-- ║  vieja. Con esto la ficha aprende del comprobante.                  ║
-- ║                                                                     ║
-- ║  Clave nueva OPCIONAL dentro de p_lineas: `aplicar_iva` (boolean).   ║
-- ║  La firma NO cambia (patrón de la v13 con `precio_venta`), así el    ║
-- ║  deploy queda desacoplado y un cliente viejo —una pestaña sin       ║
-- ║  recargar— no puede estampar su 21 sobre un producto al 10,5.       ║
-- ║                                                                     ║
-- ║  El write va FUERA del guard de p_afecta_precio_venta: la alícuota  ║
-- ║  es un dato del producto (como el costo), no una decisión de        ║
-- ║  precio. Nada más del cuerpo cambia.                                ║
-- ╚════════════════════════════════════════════════════════════════════╝
...
begin
  ...
  for v_linea in select * from jsonb_array_elements(p_lineas) loop
    ...
    -- ── v24 (mig 168): las alícuotas del comprobante vuelven a la ficha ──
    --   Hasta la v23, corregir el IVA en un renglón de la factura se perdía:
    --   el dato quedaba solo en items_factura_compra y la próxima factura
    --   volvía a proponer el de la ficha. Ahora la ficha aprende del papel.
    --   Va FUERA del guard de p_afecta_precio_venta A PROPÓSITO: la alícuota
    --   es un DATO del producto —como el costo, que también se guarda
    --   siempre—, no una decisión de precio. Apagar el switch protege el
    --   PRECIO, no esto.
    --   `aplicar_iva` es una clave OPCIONAL de p_lineas (mismo patrón que
    --   `precio_venta` en la v13 y `fecha_vencimiento` en la v12: la firma no
    --   cambia). Solo la manda el cliente que siembra los IVAs desde la
    --   ficha; uno viejo —una pestaña sin recargar— no la manda y no pisa
    --   nada. Sin esa llave estamparía su 21 hardcodeado sobre un producto
    --   que está al 10,5.
    if coalesce((v_linea->>'aplicar_iva')::boolean, false) then
      update public.productos
        set iva_venta = v_iva_venta,
            ...
            updated_at = v_ahora
        where id = v_prod_id
          -- Sin cambio real no se toca la fila (ni se mueve updated_at).
