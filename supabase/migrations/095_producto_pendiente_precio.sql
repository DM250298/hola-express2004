-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 095 · Producto pendiente de precio (alta al vuelo)        ║
-- ║                                                                     ║
-- ║  Cuando se da de alta un producto "al vuelo" desde Compras o la      ║
-- ║  Recepción (sin costo ni precio todavía), queda con                  ║
-- ║  pendiente_precio = true. En ese estado:                             ║
-- ║   · se VE en el POS pero NO se puede vender (cartel "Falta precio")  ║
-- ║   · se libera al cargar la factura con costo+precio, o al editar el  ║
-- ║     producto a mano y completar el precio de venta.                  ║
-- ║                                                                     ║
-- ║  Los productos existentes quedan pendiente_precio = false (default), ║
-- ║  o sea, sin cambios de comportamiento.                               ║
-- ║                                                                     ║
-- ║  La firma de fn_guardar_factura_compra NO cambia → CREATE OR REPLACE ║
-- ║  reemplaza limpio (no toca types/database.ts).                       ║
-- ║                                                                     ║
...
alter table public.productos
  add column if not exists pendiente_precio boolean not null default false;

comment on column public.productos.pendiente_precio is
  'Producto dado de alta al vuelo (compra/recepción) sin precio: visible pero bloqueado en el POS hasta cargar la factura o completar el precio a mano.';

-- ─── 2) fn_guardar_factura_compra v9 ────────────────────────────────
--   Idéntica a la v8 (086) salvo: al escribir el precio de venta también
--   libera el producto (pendiente_precio = false) si quedó con precio > 0.
