-- ╔════════════════════════════════════════════════════════════════════╗
-- ║  Migration 192 · stock_desfasado: acotarlo a lo que mueve la aguja  ║
-- ║                                                                     ║
-- ║  Tras la 191 quedaron 189 alertas de stock_desfasado: cierto, pero  ║
-- ║  inservible como alerta (nadie cuenta 189 productos, la lista no    ║
-- ║  se vacía y se ignora). Agarraba TODO el catálogo.                  ║
-- ║                                                                     ║
-- ║  Dos parámetros nuevos, editables desde la pantalla de reglas:      ║
-- ║   · clases           → por defecto A y B                            ║
-- ║   · solo_con_ventas  → por defecto true (vendió en 30 días)         ║
-- ║                                                                     ║
-- ║  Reemplaza fn__alertas_reglas_productos (migs 184 y 191): solo      ║
-- ║  cambia el bloque de stock_desfasado. Lo que sale del filtro se     ║
-- ║  resuelve solo como 'condicion_superada'.                           ║
-- ║                                                                     ║
-- ║  REQUIERE: migs 183-187, 190 y 191. Ejecutar UNA sola vez, COMPLETO.║
-- ╚════════════════════════════════════════════════════════════════════╝

update public.reglas_alerta
set parametros = parametros || '{"clases": ["A", "B"], "solo_con_ventas": true}'::jsonb,
    updated_at = now()
where codigo = 'stock_desfasado'
  and not (parametros ? 'clases');
...
begin
  ...
  if found then
    ...
    where s.desfasado
      -- Acotada a lo que importa: si abarca todo el catálogo, la lista no
      -- se vacía nunca y deja de mirarse.
      and (cardinality(v_clases) = 0 or s.clase_abc = any(v_clases))
      and (not coalesce((v_par->>'solo_con_ventas')::boolean, true) or s.ingresos > 0);
  end if;
...
select parametros as parametros_stock_desfasado
from public.reglas_alerta where codigo = 'stock_desfasado';
