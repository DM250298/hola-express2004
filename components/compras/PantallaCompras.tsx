...
export function PantallaCompras() {
  ...
  // Al cargar / cambiar la lista, seleccionar todo con la cantidad sugerida.
  useEffect(() => {
    if (!productos) return
  ...
  const itemsSel = useMemo(
    () =>
      (productos ?? [])
        ...
        .filter(({ p, cantidad }) => seleccion[p.id]?.seleccionado && cantidad > 0),
    [productos, seleccion]
  )

  const proveedorElegido = proveedorFiltro !== TODOS
  const nombreProveedor =
    itemsProveedor[proveedorFiltro] ?? 'Proveedor'
  ...
  function crearBorrador() {
    ...
    crearPedido.mutate(
      ...
      {
        onSuccess: () =>
          toast.success('Pedido borrador creado — revisalo en Pedidos'),
      }
    )
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5">
      <header>
        <h1 className="text-[#391511] text-2xl font-bold">Compras</h1>
